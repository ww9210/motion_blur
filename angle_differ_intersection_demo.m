...
for frame=1:25
    ...
    [label, model, llh] = emgm(magnitude_1st, k);
    %ggenerated_data = spread_1D(magnitude_1st, model);
    ...
    %% flagmap of the label generated by the gaussian mixture model EM algorithm
    RGB_gmm_flagmap = zeros(sim.frame_height,sim.frame_width,3);
    for i = 1:14400
        row = fix(i/num_in_row)+1;
        col = fix(mod((i-1),num_in_row)+1);
        switch label(i)
            case 1
                RGB_gmm_flagmap((row*8-7):(row*8),(col*8-7):(col*8),1)=1;%yello for label = 1
                RGB_gmm_flagmap((row*8-7):(row*8),(col*8-7):(col*8),2)=1;
                RGB_gmm_flagmap((row*8-7):(row*8),(col*8-7):(col*8),3)=0;
            case 2
                RGB_gmm_flagmap((row*8-7):(row*8),(col*8-7):(col*8),1)=0;%blue for label = 2
                RGB_gmm_flagmap((row*8-7):(row*8),(col*8-7):(col*8),2)=0;
                RGB_gmm_flagmap((row*8-7):(row*8),(col*8-7):(col*8),3)=1;
            case 3
                RGB_gmm_flagmap((row*8-7):(row*8),(col*8-7):(col*8),1)=1;%magenta for label = 3
                RGB_gmm_flagmap((row*8-7):(row*8),(col*8-7):(col*8),2)=0;
                RGB_gmm_flagmap((row*8-7):(row*8),(col*8-7):(col*8),3)=1;
        end
    end

    mkdir('./Result/pics','gmm_flagmap_intersection')
    imwrite(RGB_gmm_flagmap,strcat('./Result/pics/gmm_flagmap_intersection/gmm_flagmap_',num2str(frame+23),'.tiff'))
    
    %figure
    %imshow(RGB_gmm_flagmap);
