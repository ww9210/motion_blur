...
frame_last_2      =   luma{1};
frame_last_1      =   luma{2};
frame_current     =   luma{3};  % take 2 and 3 as example. 3 frames is for acceleration
...
% around
tic
frame_last_2_ups  = interpolation(frame_last_2);
...
tic
[motion_last_1,frame_last_prediction] = calc_motion(frame_last_2_ups,frame_last_1);
toc

tic
frame_last_1_ups  = interpolation(frame_last_prediction);
toc
tic
...
%% acceleration, blurring image block and compare result with original one.
acceleration = calc_acceleration(motion_last_1,motion_current);
% blurring using motion_current and acceleration and compare 
% blurring kernel generation
tic
...
[blurring_info, frame_with_blurring] = compensate_blur(frame_last_prediction, frame_current,frame_current_prediction,motion_current,acceleration);
toc

% str = './Result/blurring_info.txt';
...
%         imwrite(current,strcat('./Result/frames/',num2str(num)),'jpg');
            str = './Result/blurring_info_25_feb.txt';
            [height, width] = size(blurring_info);
            fid = fopen(str, 'wt');
            for i = 1:height
                for j = 1:width
