function [blurring_info, frame_with_blurring] = compensate_blur(...
    frame_last,frame_current,frame_current_prediction,motion_current,acceleration)
...
%     imageblock_to_be_blurred = zeros(sim.blocksize*4, sim.blocksize*4);
    ...
    % generate blurring for block
        % filters
        blur_filter_x = blurfilt(acceleration(blocknum,2), motion_current(blocknum,3));
        blur_filter_y = (blurfilt(acceleration(blocknum,3), motion_current(blocknum,4)))';
        % blurring
        blur_filter_2D = zeros(length(blur_filter_y),length(blur_filter_x));
        for l=1:length(blur_filter_y)
            blur_filter_2D(l,:) = conv(blur_filter_x, blur_filter_y(l));
        end
    % blurring image block  
    % &
    % interpolationbuchholz kleefeld
    % imageblock_blurred has 16 layers to describe 1/4 pel accuracy
        imageblock_blurred = blurimage(imageblock_to_be_blurred,blur_filter_2D, ...
            acceleration(blocknum,2),acceleration(blocknum,3));
