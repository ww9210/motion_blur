function out = interpolate_pixel(in, filter, pos)
...
% adjustment for last row/col
% if sort
%    out(sim.frame_height,:) = out(sim.frame_height-1,:);
% else
%    out(:, sim.frame_width) = out(:, sim.frame_width-1);
% end

% problems left by acceleration calculation and inaccurate motion search
% are from the boundary of right and below.
